...
vector<typename Graph::Node*> AStar::findPath(Graph& graph, string start, string goal) {
    priority_queue<pair<double, Graph::Node*>> openSet;    // use of priority queue and maps

    //vector

    vector<vector<typename Graph::Node>> adjListCopy = graph.getAdjlist();

    vector<vector<typename Graph::Node>> adjListCopy2 = graph.getAdjlist();


    Graph::Node* startNode = nullptr;
    Graph::Node* goalNode = nullptr;

    //we will traverse the whole graph to find the nodes
    for (vector<vector<typename Graph::Node>> node : adjListCopy2) { // find the satrt and goal odes based on their names.
        if (node.name == start) {
            startNode = &node;
    ...
    //We have start node and destination node from the user
    ...
    while (!openSet.empty()) {
        ...
        if (current == goalNode) { // it will check if the current node is the goal node and if it is true it will contruct the path
            ...
            vector<typename Graph::Node*> path;
